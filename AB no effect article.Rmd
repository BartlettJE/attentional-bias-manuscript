...
The trial order was randomised with each picture pair presented four times to cover each combination of image (left and right) and dot location (left and right). This combination determined the trial type condition, where a left smoking image, right nonsmoking image, and left dot would produce a smoking-nonsmoking trial where participants must identify the dot replacing the smoking image. For each picture pair, this process was repeated twice for each SOA condition, producing 384 trials split into two blocks with 64 trials in each SOA and trial type condition.
