...
Change this section to focus on why we should care about attentional bias as one paragraph. What is it related to and how is it associated with addiction? Then new paragraph on previous attempts to compare smoking groups. 
...
We used 16 image pairs from the International Affective Picture System [@lang_international_2008] for the neutral trials. We developed a series of matching smoking and non-smoking images in our lab [@bartlett_daily_2020] for the smoking and non-smoking trials. The list of IAPS images we used is available on the OSF project and our smoking/non-smoking images are available on the Gorilla open materials page.  
...
In our preregistration protocol, we focused on the attentional bias index as our outcome for confirmatory analyses, calculating it from the difference between smoking and neutral trials. While there were no significant differences between smoking groups or SOA conditions, both peer-reviewers questioned whether participants showed an attentional bias effect towards smoking images. Therefore, we performed exploratory analyses where we included trial type as an additional within-subjects IV and used response time (ms) as our outcome. 
...
# Interaction plot for 2x2x2 design
trial.plot <- afex_plot(anov_trial, 
          x = "smoke_everyday", 
          trace = "condition", 
          panel = "soa",
          error_ci = T, # Show 95% CI 
